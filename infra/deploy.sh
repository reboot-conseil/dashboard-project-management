...
$SSH "${SERVER}" "cd ${REMOTE_DIR} && DOCKER_BUILDKIT=0 docker compose build --no-cache"

echo "==> [4/5] Arrêt de l'ancienne version et démarrage..."
$SSH "${SERVER}" "cd ${REMOTE_DIR} && docker compose down && docker compose --env-file .env.production up -d"

echo "==> [5/5] Synchronisation schéma Prisma..."
$SSH "${SERVER}" "cd ${REMOTE_DIR} && docker compose exec app npx prisma@6 db push --accept-data-loss"
...
echo ""
echo "✓ Déploiement terminé — http://${SERVER}"
echo "  Logs : ssh ${SERVER} 'cd ${REMOTE_DIR} && docker compose logs -f app'"
